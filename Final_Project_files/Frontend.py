...
@st.cache_resource
def download_assets():
    """Downloads all necessary files from Google Drive."""
    st.info("Starting asset download from Google Drive...")
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        st.info("AI Model already exists locally.")

    # 2. Download Example Images
    os.makedirs(EXAMPLE_DIR, exist_ok=True)
    with st.spinner("Downloading example images..."):
    ...
    with st.spinner(f"Downloading {len(PRESET_BACKGROUND_MAPPING)} Preset Backgrounds..."):
...
# ---------------- EXAMPLE BEFORE / AFTER ----------------
example_before_path = os.path.join(DOWNLOAD_DIR, "download (33).jpg")
example_after_path = os.path.join(DOWNLOAD_DIR, "download (33)_black_bg.jpg")

example_before = Image.open(example_before_path).convert("RGB") if os.path.exists(example_before_path) else None
example_after = Image.open(example_after_path).convert("RGBA") if os.path.exists(example_after_path) else None
...
def apply_background(fg_image, bg_option, bg_color=None, bg_image_file=None, preset_bg_file=None):
    ...
    if bg_option == "Transparent":
        ...
        color_tuple = tuple(int(bg_color[i:i+2],16) for i in (1,3,5))
        bg_np[:, :, :3] = color_tuple
    elif bg_option == "Custom Background" and bg_image_file is not None:
        bg = Image.open(bg_image_file).convert("RGB").resize(fg.size)
        bg_np = np.array(bg)
    elif bg_option == "Preset Backgrounds" and preset_bg_file is not None:
        bg = Image.open(preset_bg_file).convert("RGB").resize(fg.size)
        bg_np = np.array(bg)
...
if os.path.exists(DEMO_DIR):
    ...
    st.warning(f"Demo images directory '{DEMO_DIR}' is empty or missing. Please fill in the file IDs in the script.")
...
if st.session_state.get("selected_demo") and not uploaded_any:
    ...
    try:
        ...
        st.error(f"Could not load demo image at {demo_img_path}: {e}")
elif processing_mode == "Single Image" and uploaded_file:
    image = Image.open(uploaded_file).convert("RGB")
    process_and_display(image, uploaded_file.name)
elif processing_mode == "Batch Processing" and uploaded_files:
    st.subheader("📂 Batch Subject Extraction")
