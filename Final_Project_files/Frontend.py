...
DEMO_DIR = "Demo-Image"
DEMO_FOLDER_ID = st.secrets.get("DEMO_FOLDER_ID", "1Ibc5YYMM3byWIiKbI3mACzpYbskmgs_L")
MODEL_FILE_ID = st.secrets.get("MODEL_FILE_ID", "1BW7ZpdGILFiDjnvEb0V1S4q7ZBYcwiI8")


def download_from_drive(file_id: str, dest: str, is_folder: bool = False):
    """Downloads a file or folder from Google Drive."""
    os.makedirs(os.path.dirname(dest), exist_ok=True)
    if not os.path.exists(dest):
        with st.spinner(f"Downloading {'folder' if is_folder else 'file'}..."):
            url = f"https://drive.google.com/uc?id={file_id}"
            if is_folder:
                gdown.download_folder(url, output=dest, quiet=True)
            else:
                gdown.download(url, dest, quiet=True)
        st.success(f"{'Folder' if is_folder else 'File'} downloaded successfully!")
    else:
        st.info(f"{'Folder' if is_folder else 'File'} already exists.")


# Download the model file and demo folder
download_from_drive(MODEL_FILE_ID, MODEL_PATH)
download_from_drive(DEMO_FOLDER_ID, DEMO_DIR, is_folder=True)
